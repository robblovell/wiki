...
/**
 * Check if page is not in exclude list (including by parent category)
 */
const isNotExcluded = function(item, excludeList = [], separator = '') {
    return !excludeList.some((excluded) => {
        return item.indexOf(excluded) === 0 || item.indexOf(excluded.replace(/\.md$/i, separator)) === 0;
    });
};
...
class GitWikiToHTML {
    ...
    loadFiles() {
        ...
        this.srcFiles = filenames.filter((item) => {
            ...
            // apply the exclude filter after removing locales prefix
            let langReg = this.multilang ? new RegExp(localeMatch + this.separator) : '';
            return isNotExcluded(item.replace(langReg, ''), this.rules.exclude, this.separator);
        });
